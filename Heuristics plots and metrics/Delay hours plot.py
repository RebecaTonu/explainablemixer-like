...
def get_delay_matrix(df, p_col, c_col):
    temp = df[[p_col, c_col]].copy()
    temp["p_dt"] = parse_utc(temp[p_col])
    temp["c_dt"] = parse_utc(temp[c_col])
    temp["diff"] = (temp["c_dt"] - temp["p_dt"]).dt.total_seconds() / 3600
    temp = temp.dropna(subset=["diff", "c_dt"])
    temp = temp[temp["diff"] >= 0]
    temp["year"] = temp["c_dt"].dt.year
    years = sorted(temp["year"].unique())
    flagged = pd.DataFrame(0, index=years, columns=THRESHOLDS)
    normal  = pd.DataFrame(0, index=years, columns=THRESHOLDS)
    for t in THRESHOLDS:
        sub_flag = temp[temp["diff"] <= t]
        sub_norm = temp[temp["diff"] > t]
        flagged[t] = sub_flag.groupby("year").size()
        normal[t]  = sub_norm.groupby("year").size()
    return flagged.fillna(0), normal.fillna(0), years
dfs = [pd.read_csv(f) for f in DATA_DIR.glob("*.csv")]
full_df = pd.concat(dfs, ignore_index=True)
s_f, s_n, years = get_delay_matrix(full_df, S_PREV, S_CURR)
r_f, r_n, _     = get_delay_matrix(full_df, R_PREV, R_CURR)
